...
module.exports = {
	name: 'gives',
	description: 'Gibt einem beliebigen User eine beliebige Anzahl an Seelen',
	category: 'Admin',
    usage: '``c!gives @user [anzahl]``',
	execute(arguments, receivedMessage) {
		if (receivedMessage.author.id == "422716344228642828"){
			...
			con.query("SELECT id ,coins, coinsget FROM userdata WHERE usrid = '"+target.id+"'", function (err, result, fields) {
				...
				receivedMessage.channel.send(""+target+" wurden **"+givec+"**❦ geschenkt.")
				const lt = require('./../bot.js');
				lt.logging("0x0000FF", "Währungssystem", "Ein Logeintrag zum Währungssystem", "Seelen wurden gegeben", 
						""+receivedMessage.author+" (ID: ``"+receivedMessage.author.id+"``) hat "+target+" (ID: ``"+target.id+"``) **"+givec+"**❦ geschenkt.");
			});
		    }
	},
};