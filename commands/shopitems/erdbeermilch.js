...
const wert = 1250
const name = "Erdbeermilch"

let buyerdbeermilch = function(mes) {
	con.query("SELECT id, coins, power, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		...
		if (aktucoins >= wert) {
            ...
            con.query("UPDATE `cerbotdb`.`userdata` SET `power`=" + (aktupower + 5) + " WHERE  `id`="+result[0].id+";")
			mes.channel.send(mes.author+" du hast das Item ``"+name+"`` **erfolgreich gekauft**. Dir wurden **"+wert+"**❦ genommen. Deine Angriffspunkte wurden um **5** Punkte auf **"+(aktupower + 10)+"** erhöht!")
...
exports.buyerdbeermilch = buyerdbeermilch;