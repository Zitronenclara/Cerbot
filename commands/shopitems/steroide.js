...
let buysteroide = function(mes) {
	con.query("SELECT id, coins, power, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		...
		if (aktucoins >= wert) {
            ...
            con.query("UPDATE `cerbotdb`.`userdata` SET `power`=" + (aktupower + 1) + " WHERE  `id`="+result[0].id+";")
			mes.channel.send(mes.author+" du hast das Item ``"+name+"`` **erfolgreich gekauft**. Dir wurden **"+wert+"**❦ genommen. Deine Angriffspunkte wurden um **1** Punkt auf **"+(aktupower + 6)+"** erhöht!")
...
exports.buysteroide = buysteroide;