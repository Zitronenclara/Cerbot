...
const wert = 150000
const name = "Kekselaos"
const roleid = "652839905679900672"

let buykekselaos = function(mes) {
...
let sellkekselaos = function(mes) {
	con.query("SELECT id ,coins, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		if (err) throw err;
		if (mes.member.roles.has(roleid)){
			var aktucoins = result[0].coins;
...
exports.buykekselaos = buykekselaos;
exports.sellkekselaos = sellkekselaos;