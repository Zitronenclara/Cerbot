...
const wert = 1350
const name = "Vibranium"

let buyvibranium = function(mes) {
	con.query("SELECT id, coins, defense, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		...
		if (aktucoins >= wert) {
            ...
            con.query("UPDATE `cerbotdb`.`userdata` SET `defense`=" + (aktudef + 3) + " WHERE  `id`="+result[0].id+";")
			mes.channel.send(mes.author+" du hast das Item ``"+name+"`` **erfolgreich gekauft**. Dir wurden **"+wert+"**❦ genommen. Deine Verteidigungspunkte wurden um **3** Punkte auf **"+(aktudef + 3)+"** erhöht!")
...
exports.buyvibranium = buyvibranium;