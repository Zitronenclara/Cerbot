...
let buycalcium = function(mes) {
	con.query("SELECT id, coins, maxhp, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		...
		if (aktucoins >= wert) {
            ...
            con.query("UPDATE `cerbotdb`.`userdata` SET `maxhp`=" + (aktumaxhp + 5) + " WHERE  `id`="+result[0].id+";")
			mes.channel.send(mes.author+" du hast das Item ``"+name+"`` **erfolgreich gekauft**. Dir wurden **"+wert+"**❦ genommen. Deine maximalen HP wurden um **5** Punkte auf **"+(aktumaxhp + 5)+"** erhöht!")
