...
const wert = 15000
const name = "Hephaistos"
const roleid = "633320104536571905"

let buyhephaistos = function(mes) {
...
let sellhephaistos = function(mes) {
	con.query("SELECT id ,coins, coinsspent FROM userdata WHERE usrid = '"+mes.author.id+"'", function (err, result, fields) {
		if (err) throw err;
		if (mes.member.roles.has(roleid)){
			var aktucoins = result[0].coins;
...
exports.buyhephaistos = buyhephaistos;
exports.sellhephaistos = sellhephaistos;